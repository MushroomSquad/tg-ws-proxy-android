...
#ifdef ANDROID_APP
extern int vpn_protect_fd(int fd);
#endif
...
#ifdef ANDROID_APP
    /* Bypass VPN TUN for ByeDPI outbound sockets to avoid routing loops
       while allowing other app sockets (TgWsProxy) to go through the VPN. */
    (void)vpn_protect_fd(fd);
#endif
    #ifdef _WIN32
